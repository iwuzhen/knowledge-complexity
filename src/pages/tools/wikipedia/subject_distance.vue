...
import { getWikipediaSubjectDistance } from '~/api/methods/wikinikepai'
...
const SELECT_Wikipedia_subject = ['Biology', 'Computer science', 'Economics', 'Engineering disciplines', 'Environmental science', 'Geography', 'Geology', 'Linguistics', 'Materials science', 'Mathematics', 'Medicine', 'Philosophy', 'Physics', 'Political science', 'Psychology', 'Sociology']

const subjectOption = SELECT_Wikipedia_subject.map((item) => {
  return {
    label: item,
    value: item,
  }
...
const guidNationText = computed(() => {
  return '使用 Google 距离计算 wikipedia 中的各个学科的距离，值越接近0，学科越相关'
})

const pageTags = ref('subject')

const { loading: paperLoading, onSuccess } = useWatcher(
  () => getWikipediaSubjectDistance(subjectSelected.value),
  ...
  <n-grid cols="10" x-gap="12" item-responsive responsive="screen">
    <n-gi span="8 m:4 l:3">
      <n-tabs v-model:value="pageTags" type="segment">
        <n-tab-pane name="subject" tab="wikipedia 学科">
          <n-alert type="info" :show-icon="false" mb-5>
            {{ guidNationText }}
          </n-alert>
          <n-form-item path="subject" label="选择学科:">
    ...
    <n-gi span="8 m:6 l:7">
      <n-tabs justify-content="space-evenly" type="line">
        <n-tab-pane name="chap2" tab="wikipedia 学科距离">
          <VChart
