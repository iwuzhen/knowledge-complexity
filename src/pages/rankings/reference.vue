...
import { get_number_of_papers_per_year_by_country_dx } from '~/api/methods/complexity'
...
const chartType = ref('paper')
const chartTypeOption = ref([
  {
    value: 'paper',
    label: t('ranking.release'),
    chartTitle: '国家发表的论文逐年趋势',
  },
  {
    value: 'import',
    label: t('ranking.import'),
    chartTitle: '国家进口的论文逐年趋势',
  },
  {
    value: 'export',
    label: t('ranking.export'),
    chartTitle: '国家出口的论文逐年趋势',
  },
])

const guidNationText = computed(() => {
  if (chartType.value === 'paper')
    return '各国发表论文数量排名随时间变化情况'

  else if (chartType.value === 'import')
    return '各国发表论文引用其他国家论文数量的排名随时间变化情况'

  else if (chartType.value === 'export')
    return '各国发表论文被其他国家引用的数量排名随时间变化情况'
})
...
const { loading, data: chartData } = useWatcher(
  () => get_number_of_papers_per_year_by_country_dx(chartType.value),
  // 被监听的状态数组，这些状态变化将会触发一次请求
  [chartType],
  {
    // 设置500ms防抖，如果keyword频繁变化，只有在停止变化后500ms才发送请求
    debounce: 500,
    initialData: { legend: [], rank: [], year: [2000, 2010, 2020], data: [] },
    immediate: true,
  },
)
    ...
    <n-gi span="8 m:6 l:7">
      <n-tabs justify-content="space-evenly" type="line">
        <n-tab-pane name="chap1" tab="排名图">
          <countryBumpCombination :data="chartData" :i18n="true" :value-fixed="0" />
        </n-tab-pane>
