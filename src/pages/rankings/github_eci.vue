...
import { get_github_country_eci } from '~/api/methods/complexity'
...
const { loading, data: chartData } = useWatcher(
  () => get_github_country_eci(filter_cat.value),
  [filter_cat],
...
// watchEffect(() => {
//   const yearIndex = chartData.value.year.indexOf(mapYear.value)
//   const countryValues = chartData.value.data.map((row) => {
...
//       orient: 'horizontal',
//       precision: 4,
//       min: Math.min(...countryValues),
//       max: Math.max(...countryValues),
//       inRange: {
//         color: [
//           '#313695',
//           '#4575b4',
  ...
  <n-grid cols="10" x-gap="12" item-responsive responsive="screen">
    ...
    <n-gi span="8 m:6 l:7">
      <n-tabs justify-content="space-evenly" type="line">
        <n-tab-pane name="chap1" tab="排名图" :loading="loading">
          <n-spin :show="loading">
            <countryBumpCombination :data="chartData" :i18n="true" :year-default="[2016, 2018, 2020]" :value-fixed="3" />
          </n-spin>
        </n-tab-pane>
